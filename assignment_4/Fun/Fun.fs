...
let rec eval (e : expr) (env : value env) : int =
    ...
    | Letfun(f, x, fBody, letBody) -> 
      ...
      eval letBody bodyEnv
    | Call(Var f, eArgs) -> (* (From assignment 3) CHANGED | 4.3*)
      let fClosure = lookup env f
      match fClosure with
      | Closure (f, x, fBody, fDeclEnv) -> (* (From assignment 3) CHANGED | 4.3 *)
    ...
    | Fun _ ->  failwith "eval Call: use HigherFun.fs lang for lambda functions" (* CHANGED | 4.4 *)
...
let ex7 = 
    Letfun("max2", ["a"; "b"], If (Prim ("<", Var "a", Var "b"), Var "b", Var "a"),
     Letfun
       ("max3", ["a"; "b"; "c"],
