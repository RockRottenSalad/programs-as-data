...
type expr = 
  | CstI of int
  | Var of string
  | Let of (string * expr) list * expr (* 1.2.1 *)
...
// Converts an sintr into its machine code corresponding value. The values are the ones from Machine.java and SCstI and Var also carries their data
let parseSinstr = (* Changed *)
    function
    | SCstI x -> [0; x]
    | SVar x -> [1; x]
    | SAdd -> [2]
...
let rec assemble  = List.collect parseSinstr  (* Changed *)

let ecomp expr = scomp expr [] |> assemble (* Changed, compiles exprs to scomp list which is then passed to assemble*)
let ecompp expr = (* Changed, now also outputs to a file *)
    let res = scomp expr [] |> assemble
    intsToFile res "idk"
    res
(* -----------------------------------------------------------------  *)