...
type value = 
  | Int of int
  | Closure of string * string list * expr * value env       (* Changed *)
...
let rec eval (e : expr) (env : value env) : int =
    ...
    | Letfun(f, x, fBody, letBody) -> 
      ...
      eval letBody bodyEnv
    | Call(Var f, eArguments) -> 
      let fClosure = lookup env f
      match fClosure with
      | Closure (f, xs, fBody, fDeclEnv) ->
        let idk = List.zip xs eArguments
        let variables = List.map (fun (x, eArg) -> (x, Int(eval eArg env))) idk
        let fBodyEnv = variables @ (f, fClosure) :: fDeclEnv
        eval fBody fBodyEnv
